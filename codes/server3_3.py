...
# 출력 순서 지정
FEATURE_ORDER = ['voltage', 'vibration']  # 출력 순서를 원하는 키로 명시적으로 고정
...
def format_log(log):
    ...
    formatted_log = f"Timestamp: {timestamp}\n"
    
    anomalies = log['anomalies']
    
    # FEATURE_ORDER에 따라 출력
    for feature in FEATURE_ORDER:
        if feature in anomalies:
            formatted_log += format_feature(feature, anomalies[feature])
    
    # FEATURE_ORDER에 없는 추가 feature는 알파벳 순서로 출력
    remaining_features = sorted([f for f in anomalies if f not in FEATURE_ORDER])
...
# 개별 feature 포맷팅
def format_feature(feature_name, details):
    formatted = f"  {feature_name.capitalize()}:\n"
    
    # is_anomaly 색상: 초록색 (32)
    is_anomaly_color = "31" if details['is_anomaly'] else "32"
    formatted += f"    is_anomaly: {color_text(details['is_anomaly'], is_anomaly_color)}\n"
    
    # reconstruction_error 색상: 빨간색 (31) 또는 초록색 (32)
    reconstruction_error_value = f"{details['reconstruction_error']:.3f}"  # 값 미리 계산
    reconstruction_color = "31" if details['reconstruction_error'] > details['threshold'] else "32"
    formatted += f"    reconstruction_error: {color_text(reconstruction_error_value, reconstruction_color)}\n"

    # threshold와 actual_value는 기본 색상
    formatted += f"    threshold: {details['threshold']:.3f}\n"
    formatted += f"    actual_value: {details['actual_value']:.3f}\n"
...
class MultiModelAnomalyDetector:
    ...
    def process_data(self, data):
        ...
        if self.results_ready.wait(timeout=0.8):
            ...
            print('============================================================')
            print(format_log(combined_result))
            ...
            for feature_type, anomaly_result in self.results.items():
                if anomaly_result['is_anomaly']:
                    ...
                    utc_time = datetime.utcfromtimestamp(anomaly_result['timestamp'] / 1000)
                    ...
                    alarm_message = {
                        ...
                        'value': actual_value, 
                        'createAt': kst_time.strftime("%Y-%m-%d %H:%M:%S"),
...
def main():
    selected_features = ['vibration', 'voltage']  # 진동 데이터만 사용
